...
document.addEventListener('DOMContentLoaded', () => {
  ...
  //
  // Separately, an n8n webhook sends a personalized HTML auto-reply to the
  // CLIENT (not to you). Set N8N_AUTOREPLY_WEBHOOK_URL below once your n8n
  ...
  if (contactForm) {
    ...
    // Fire-and-forget: triggers the n8n workflow that emails the client a
    // personalized auto-reply. Never blocks or fails the main form submit.
    const triggerClientAutoReply = (payload) => {
      if (!N8N_AUTOREPLY_WEBHOOK_URL || N8N_AUTOREPLY_WEBHOOK_URL.includes('YOUR-N8N-INSTANCE')) {
        return; // not configured yet — silently skip
      }
      fetch(N8N_AUTOREPLY_WEBHOOK_URL, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload),
      }).catch((err) => {
        console.warn('n8n auto-reply trigger failed (non-blocking):', err);
      });
    };
    ...
    contactForm.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        if (result.success) {
          triggerClientAutoReply(payload); // send the client their auto-reply via n8n
          contactForm.reset();
